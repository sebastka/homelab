resource "proxmox_virtual_environment_container" "containers_database" {
  for_each = { for k, v in var.containers : k => v if v.role == "database" }
  # vm_id        = each.value.vm_id

  node_name   = var.pve.name
  description = each.value.description
  tags        = concat(["lxc", "linux", "database"], each.value.tags)
